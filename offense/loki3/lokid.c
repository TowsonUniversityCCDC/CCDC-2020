    ...
    for (;;) {
	    ...
	    switch (pid = fork()) {
            ...
            if (rdg.payload[0] == L_PK_REQ) {
                ...
                {
#endif /* STRONG_CRYPTO */              /* MAX_CLIENT limit reached */
                    lokid_xmit((u_char *)S_MSG_PACKED, LP_DST, L_ERR, NOCR);
...
#else
                DH_get0_key(dh_keypair, &pub_key, &priv_key);
                BN_bn2bin(pub_key, buf1);
            ...
            if ((c_ind = locate_client(FIND)) == -1) {
                ...
                lokid_xmit((u_char *)S_MSG_UNKNOWN, LP_DST, L_ERR, NOCR);
                lokid_xmit(buf1, LP_DST, L_EOT, NOCR);
...
void d_parse(u_char *buf, pid_t pid, int ripsock) {
    ...
    lokid_xmit((u_char *)S_MSG_UNSUP, LP_DST, L_REPLY, OKCR);
...
void swap_t(int signo) {
    ...
    while (n < MAX_CLIENT) {
        if ((client_ip = check_client_ip(n++, &c_id))) {
            ...
            lokid_xmit((u_char *)buf, client_ip, L_REPLY, OKCR);
            lokid_xmit((u_char *)buf, client_ip, L_EOT, OKCR);
/*            update_client(locate_client(FIND), p_sent, b_sent);*/
        }
    }
    ...
    lokid_xmit((u_char *)buf, LP_DST, L_REPLY, OKCR);
    lokid_xmit((u_char *)buf, LP_DST, L_EOT, OKCR);
    update_client(locate_client(FIND), p_sent, b_sent);
